...
static void print_execve_help(const Tracee *tracee, const char *argv0, int status)
{
	...
	/* linuxdroid-exec replaced execve with path with one that doesn't exist inside proot?  */
	if (status == -ENOENT && getenv("LD_PRELOAD") != NULL && strstr(getenv("LD_PRELOAD"), "liblinuxdroid-exec.so") != NULL) {
		note(tracee, INFO, USER,
"It seems that linuxdroid-exec is active and is prepending /data/data/com.linuxdroid/... to executable paths\n"
